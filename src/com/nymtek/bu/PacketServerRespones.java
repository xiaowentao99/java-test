...
public class PacketServerRespones extends Thread {

	static Logger logger = Logger.getLogger(PacketServerRespones.class.getName());
...
//		DatagramPacket dataPacket = new DatagramPacket(sendBuf, sendBuf.length, RemoteAddress, RemotePort);
//		serverSocket.send(dataPacket);
//	}

	public PacketServerRespones(int port) {
	...
	public void run() {
		...
		while (true) {
			try {
				...
				//int no = Tools.byteToInteger(rData, 2, 4);
				 data.setData(rData, 2, 4);
				 serverSocket.receive(data);
				synchronized (Lock) {
	...
	class PacketParseHandle extends Thread {
		...
		public void run() {
			...
			while (true) {
				...
				synchronized (Lock) {
					if(PacketServerRespones.this.packets.size() > 0)
						d = PacketServerRespones.this.packets.remove(0);
				}
