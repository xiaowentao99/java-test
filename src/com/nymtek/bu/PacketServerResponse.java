...
public class PacketServerResponse extends Thread {

	static Logger logger = Logger.getLogger(PacketServerResponse.class.getName());
	...
	public PacketServerResponse(int port) {
	...
	class PacketParseHandle extends Thread {
		...
		public void run() {
			...
			while (true) {
				...
				synchronized (Lock) {
					if(PacketServerResponse.this.packets.size() > 0)
						d = PacketServerResponse.this.packets.remove(0);
				}
